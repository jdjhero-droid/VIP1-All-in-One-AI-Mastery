 ...
 * 호출 시점에 인스턴스를 생성하여 최신 API 키가 반영되도록 합니다.
 */
const getAIClient = () => {
...
/**
 * 지침에 따라 API 키 유효성 검사 로직을 간소화하거나 
 * 'Requested entity was not found' 에러 처리를 위한 인터페이스를 제공합니다.
 */
...
export const generateStoryStructure = async (
  ...
  const systemInstruction = `
    ...
    5. 'musicPrompt': Generate a detailed billboard-style music prompt in English using EXACTLY this structure:
       ...
       Mix: 
       Language:
    6. 'lyrics': Full song lyrics in English with structure:
       [Verse 1]
       [Pre-Chorus]
       [Chorus]
       [Chorus – Repeat]
